...
#define SECONDS 4.0						// time for weaponbox to disapear (seconds) 

// weapon offsets for AG
...
new const m_iDefaultAmmo = 40;			// default ammo offset
new const weaponOffset 	 = 4;			// weapon offset 
// ================================================================================== //
...
new playerJoinMsg[ 33];					// variable to store the player join message status
...
// Fire map room 1 coordinates for bots spawn
...
// Fire map room 2 coordinates for bots spawn
...
public plugin_init() 
{
	...
	if (ag_gamemode[0] && !(equali(ag_gamemode, "train") ))
	{
		server_print("[AIM Training Plugin] The %s plugin can only be run in the ^"train^" gamemode on Adrenaline Gamer 6.6 or its Mini version for HL.", PLUGIN);
		pause("ad");
		return;
	}
...
public playerSpawn(id)
{
	...
	if((g_gFireRoom == 1) && (is_user_bot(id)))
	{
		...
		if (is_user_alive(id)) {
			...
			set_user_bpammo( id,HLW_SHOTGUN,250); 
			}
	}
	...
	if((g_gRoom == 1) && (is_user_bot(id)))
	{
		...
		if (is_user_alive(id)) {
			give_item( id, "weapon_gauss" );
			set_user_bpammo( id,HLW_GAUSS,999); 	
	...
	if((g_gRoom == 2) && (is_user_bot(id)))
	{
			...
			give_item( id, "weapon_gauss" );
			set_user_bpammo( id,HLW_GAUSS,99); 	
	}
	...
	if((g_gRoom_aim == 3) && (is_user_bot(id)))
	{
		...
		set_user_bpammo(id,HLW_CROSSBOW,99);
		give_item( id, "item_longjump" );
	}
...
public addToFullPackPost(es, e, ent, host, hostflags, player, pSet)
{
	...
	if((1<=host<=MaxPlayers) && is_valid_ent(ent) && g_CheckWh[host])
